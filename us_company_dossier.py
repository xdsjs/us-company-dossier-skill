...
class USCCompanyDossier:
    ...
    def update_dossier(self, ticker: str, years: int = 3, forms: List[str] = None,
                      include_ir: bool = True, max_filings_per_form: int = 50,
                      normalize_level: str = "light", fetch_mode: str = "http",
                      download_mode: str = "links_only") -> Dict:
        """Update an existing dossier with new filings (incremental)."""
        # For now, just call build_dossier with force_rebuild=False
        # In a production implementation, this would check the manifest
        # and only fetch filings newer than the latest one
    ...
    def status(self, ticker: str) -> Dict:
        ...
        return {
            ...
            "forms": list(set(a.get("form") for a in artifacts if a.get("form")))
        }

    def list_artifacts(self, ticker: str, filters: Dict = None) -> List[Dict]:
        """List artifacts in a company dossier with optional filters."""
        if filters is None:
            filters = {}
        ...
        for artifact in artifacts:
            # Filter by form type
            if "form" in filters and artifact.get("form") != filters["form"]:
                continue
            
            # Filter by artifact type
            if "type" in filters and artifact.get("type") != filters["type"]:
                continue
            
            # Filter by date
            if "since" in filters:
                filed_at = artifact.get("filed_at")
                if not filed_at or filed_at < filters["since"]:
                    continue
